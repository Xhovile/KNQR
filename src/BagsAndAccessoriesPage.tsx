...
interface BagsAndAccessoriesPageProps {
  products: Product[];
...
export default function BagsAndAccessoriesPage({
  ...
  // Filter bags & accessories products
  const bagsProducts = useMemo(() => {
    return products.filter(
      (p) =>
        (p.collectionCategory || "").toLowerCase() === "bags & accessories" ||
        (p.collectionCategory || "").toLowerCase() === "accessories"
    );
  }, [products]);
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          <div className="lg:col-span-7 space-y-6">
            <h1 className="font-serif text-4xl sm:text-5xl lg:text-6xl font-normal text-chocolate leading-tight tracking-tight">
              Bags &
              <span className="block italic text-gold font-light mt-1 font-serif">Accessories</span>
            </h1>
          </div>
        ...
        {filteredAndSortedProducts.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center text-center py-24 px-6 border border-chocolate/10 rounded-2xl bg-white/20 space-y-4">
            ...
            <p className="text-xs text-chocolate/60 max-w-xs mx-auto leading-relaxed font-light">
              No matching bags or accessories are in our showroom catalog. Try adjusting your subcategory or search keywords.
            </p>
          </div>
        )}
      </div>
