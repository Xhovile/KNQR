...
const CATEGORY_OPTIONS_BY_COLLECTION: Record<string, string[]> = {
...
function getCategoryDefaults(category: string): ProductDraftValues {
  switch (category) {
    case "Bags & Accessories":
      return createEmptyBagsProductDraft();
    case "Accessories":
      return createEmptyAccessoriesProductDraft();
    case "Fragrances":
...
function mapInitialValues(initialValues?: Partial<ProductDraftValues> | Product): ProductDraftValues {
  ...
  const category = (prod.collectionCategory as string) || "Apparel";
  const categoryDefaults = getCategoryDefaults(category);
  const categorySubCategory = CATEGORY_OPTIONS_BY_COLLECTION[category]?.[0] || defaultDraft.category;
  ...
  return {
    ...
    collectionCategory: category as any,
    ...
    accessoryType: (prod as any).accessoryType || categoryDefaults.accessoryType,
    accessoryMaterial: (prod as any).accessoryMaterial || categoryDefaults.accessoryMaterial,
    accessoryStyle: (prod as any).accessoryStyle || categoryDefaults.accessoryStyle,
    accessoryUseCase: (prod as any).accessoryUseCase || categoryDefaults.accessoryUseCase,
...
function buildPreviewRows(values: ProductDraftValues): Array<{ label: string; value: string }> {
  ...
  if (values.collectionCategory === "Apparel") {
    ...
    rows.push(
      { label: "Accessory Type", value: values.accessoryType || "—" },
      { label: "Material Finish", value: values.accessoryMaterial || "—" },
      { label: "Style Profile", value: values.accessoryStyle || "—" },
      { label: "Primary Use", value: values.accessoryUseCase || "—" },
      { label: "Accent Colors", value: values.colors?.length ? values.colors.join(", ") : "—" },
    );
  ...
  const handleChange = (key: string, val: any) => {
    setValues((prev) => {
      if (key === "collectionCategory") {
        const nextCategory = val as string;
        const nextDefaults = getCategoryDefaults(nextCategory);
        const nextSubCategory = CATEGORY_OPTIONS_BY_COLLECTION[nextCategory]?.[0] || prev.category;

        return {
          ...prev,
          ...nextDefaults,
          collectionCategory: nextCategory as any,
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      for (let i = 0; i < mediaFiles.length; i++) {
        ...
        const secureUrl = await uploadToCloudinary(mediaFiles[i]);
    ...
    <div className="min-h-screen bg-light-brown text-chocolate flex flex-col font-sans" id="product-form-root">
      <div className="relative md:sticky top-0 z-40 bg-white/40 border-b border-chocolate/10 backdrop-blur-md py-4 px-6 md:px-12 flex items-center justify-between text-chocolate">
        <div className="flex items-center space-x-4">
          <button type="button" onClick={onCancel} className="p-2 hover:bg-chocolate/5 rounded-full text-chocolate/70 hover:text-gold transition-colors cursor-pointer" id="product-form-back-btn" title="Cancel and return">
            <Undo className="w-5 h-5" />
          </button>
          <div>
            <span className="text-[10px] font-mono tracking-[0.3em] text-gold uppercase font-bold">KNQR Curator</span>
            <h1 className="text-xl font-serif text-chocolate tracking-wide">{mode === "create" ? "Add New Product" : `Edit ${values.name || "Product"}`}</h1>
          </div>
        </div>

        <div className="flex items-center space-x-2">
          <div className="hidden lg:flex p-1 bg-white/40 border border-chocolate/10 rounded-xl space-x-1">
            <button type="button" onClick={() => setPreviewTab("edit")} className={`px-4 py-1.5 text-xs tracking-wider rounded-lg uppercase transition-all cursor-pointer select-none ${previewTab === "edit" ? "bg-chocolate text-cream font-semibold" : "text-chocolate/50 hover:text-chocolate"}`}>Editor</button>
            <button type="button" onClick={() => setPreviewTab("preview")} className={`px-4 py-1.5 text-xs tracking-wider rounded-lg uppercase transition-all cursor-pointer select-none ${previewTab === "preview" ? "bg-chocolate text-cream font-semibold" : "text-chocolate/50 hover:text-chocolate"}`}>Live Preview</button>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 w-full max-w-7xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className={`space-y-6 lg:col-span-7 ${previewTab === "preview" ? "hidden lg:block" : "block"}`}>
          {activeSchema.sections.map((section) => {
            ...
            return (
              <FormAccordion key={section.key} sectionKey={section.key} title={section.title} description={section.description} isOpen={!!openSections[section.key]} onToggle={() => handleToggleSection(section.key)} hasError={hasError}>
                {section.key === "media" ? (
                              ...
                              <div key={`${url}-${idx}`} className={`overflow-hidden rounded-xl border bg-slate-50 ${isPrimary ? "border-gold ring-1 ring-gold/20" : "border-slate-200"}`}>
                                ...
                                <div className="border-t border-slate-200 px-2 py-2 space-y-2">
                                  <div className="flex items-center justify-between gap-2">
                                    ...
                                    {isPrimary && <span className="rounded-full bg-gold/10 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-gold">Cover</span>}
                                  </div>
                                  <div className="flex gap-2">
                                    <button type="button" onClick={() => setPrimarySavedImage(url)} className="flex-1 rounded-lg border border-slate-300 px-2 py-1 text-[11px] font-medium text-slate-700 transition hover:bg-slate-100">Set primary</button>
                                    <button type="button" onClick={() => removeSavedImage(url)} className="flex-1 rounded-lg border border-rose-200 px-2 py-1 text-[11px] font-medium text-rose-600 transition hover:bg-rose-50">Remove</button>
                                  </div>
                                </div>
                              </div>
                            );
                  ...
                  <div className="space-y-5">
                    {activeFields.map((field) => {
                      ...
                      if (field.key === "collectionCategory") {
                        fieldWithDynamicOptions = { ...field, options: COLLECTION_OPTIONS };
                      } else if (field.key === "category") {
                        fieldWithDynamicOptions = { ...field, options: CATEGORY_OPTIONS_BY_COLLECTION[values.collectionCategory as string] || [] };
                      }

                      return <FormField key={field.key} field={fieldWithDynamicOptions} value={values[field.key as keyof ProductDraftValues]} onChange={(val) => handleChange(field.key, val)} error={errors[field.key]} />;
                    })}
                  </div>
                )}
              </FormAccordion>
        ...
        <div className={`lg:col-span-5 lg:sticky lg:top-28 self-start space-y-6 ${previewTab === "edit" ? "hidden lg:block" : "block"}`}>
          <div className="bg-chocolate-dark border border-cream/15 rounded-2xl p-6 shadow-2xl space-y-6 text-cream">
            ...
            <div className="border-t border-cream/10 pt-4 space-y-3.5 text-xs">
              {previewRows.map((row) => (
                <div key={row.label} className="flex justify-between gap-4"><span className="text-cream/50">{row.label}:</span><span className="font-mono text-cream font-medium text-right">{row.value}</span></div>
              ))}
      ...
      <AnimatePresence>
        {isUploading && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-chocolate-dark/95 backdrop-blur-md p-6" id="cloudinary-upload-overlay">
